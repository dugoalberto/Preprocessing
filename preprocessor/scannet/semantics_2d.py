...
from Preproccessing.preprocessor.scannet.imgToClip import extract_clip_feature_map
...
def main(cfg : DictConfig) -> None:
    ...
    if cfg.save_clip_features:
        clip_model = OpenCLIPNetwork(OpenCLIPNetworkConfig()).eval()
        clip_features_dir = save_dir / "clip_features"
    ...
    for scene_id in tqdm(scene_list, desc='scene'):
        ...
        for image_ndx, image_name in enumerate(tqdm(image_list, desc='image', leave=False)):
            ...
            if cfg.save_clip_features:
                clip_feat = extract_clip_feature_map(
                    img,
                    pix_obj_ids,
                    clip_model,
                    bbox_expand_factor=cfg.get("bbox_expand_factor", 1.2),
                    mask_background=cfg.get("clip_mask_background", False),
                )
                out_path = clip_features_dir / scene_id / f"{image_name}.pth"
                out_path.parent.mkdir(parents=True, exist_ok=True)
                torch.save(clip_feat.half(), out_path)
